...
export function AdminView() {
  ...
  useEffect(() => {
    function calculatePercentages(tags) {
      let percentageObj = {}
      for (let tag of tags) {
        let tagCount = allTasks.filter((task) =>
          task.tags.find((tagItem) => tagItem == tag)
        ).length
        ...
        <div className="grid grid-cols-8 gap-4 my-8">
          ...
          <div className="col-span-2 row-span-2 h-auto bg-white rounded-lg">
            {barLoaded ? (
              <>
                <BarGraph
                  title={"Total Tasks Completed Per Week"}
                  labels={barLabels}
                  data={barData}
                  colors={debuggoBlue}
                />
                    ...
                    <div key={tag}>
                      <div className="justify-between">
                        ...
                        <div className="flex flex-row">
                          ...
                          />
                          <span>
                            {Math.round(percentageObject[tag] * 100)}%
                          </span>
                        </div>
                      </div>
                    </div>
