    ...
    <>
      <div className="pl-80 flex justify-between items-center bg-off-white h-16">
        <div className="text-3xl flex">
          ...
          {showButton ? (
            <div className="flex flex-row text-lg ml-12">
              ...
              <button
                className="flex border-2 ml-2 rounded-lg justify-center items-center p-2 h-8"
                onClick={() => changeView()}
              >
                {view}
              </button>
        ...
        <div className="flex flex-row justify-between rounded-lg bg-white m-2">
          <div
            className="flex flex-row border-2 rounded-lg cursor-pointer"
          ...
          {dropdown ? (
            <div className="flex flex-col w-48 rounded-lg border-2 absolute right-4 z-10 mt-12 text-xl">
              <Link
                ...
                className="flex items-center bg-white rounded-t-lg p-1 border-b-2 hover:bg-off-white h-8 cursor-pointer"
              >
                My Profile
              </Link>
              <span
                onClick={() => handleLogout()}
                className="flex items-center bg-white hover:bg-off-white h-8 rounded-b-lg p-1 cursor-pointer"
              >
                Logout
              </span>
            </div>
                ...
                return (
                  <li key={index} className="flex justify-center">
                    <NavLink
                      to={data.path}
                      className={({ isActive }) =>
                        "w-72 m-4 mt-2 mb-2 p-3 flex justify-start align-center rounded-lg " +
                        (isActive ? "bg-blue " : " hover:bg-blue bg-white")
                      }
                    >
                      {data.icon}
                ...
                return (
                  <li key={index} className="flex justify-center">
                    <NavLink
                      to={data.path}
                      className={({ isActive }) =>
                        "w-72 m-4 bg-white mt-2 mb-2 p-3 flex justify-start align-center rounded-lg " +
                        (isActive ? " bg-blue " : " hover:bg-blue")
                      }
                    >
                      {data.icon}
                      <span className="ml-4">{data.title}</span>
